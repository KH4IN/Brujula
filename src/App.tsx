...
export default function App(){
  ...
  const wealth=balances.bank+balances.cash+marketValue(investments);
  ...
  function exportCSV(){
    const rows=[['Fecha','Tipo','Cuenta','Destino','Categoría','Descripción','Importe (EUR)'],...monthly.map(x=>[x.occurred_on,x.kind==='income'?'Ingreso':x.kind==='transfer'?'Traspaso':'Gasto',x.account==='cash'?'Efectivo':'Banco',x.to_account==='cash'?'Efectivo':x.to_account==='bank'?'Banco':'',x.category,x.description,(x.kind==='income'?'':'-')+Number(x.amount).toFixed(2)])];
  ...
  return <div className="app">
    ...
    <main className="main">
      <header className="topbar"><button className="mobile-menu icon-button" aria-label="Abrir menú" onClick={()=>setMobileOpen(true)}><Menu size={23}/></button><div className="breadcrumb">Mi espacio <ChevronRight size={14}/><strong>{({dashboard:'Vista general',transactions:'Movimientos',budgets:'Presupuestos',accounts:'Cuentas y efectivo',goals:'Objetivos',investments:'Inversiones'} as Record<Tab,string>)[tab]}</strong></div><div className="topbar-right"><span className="online-dot"/> Tu dinero, en orden <span className="top-avatar">{user?.email?.charAt(0).toUpperCase()??'T'}</span></div></header>
      <div className="content">
        ...
        {tab==='dashboard'&&<>
          <div className="wealth-strip"><div><span>PATRIMONIO ESTIMADO</span><strong>{money(wealth)}</strong><small>Banco + efectivo + inversiones</small></div><button onClick={()=>go('accounts')}>Banco <strong>{money(balances.bank)}</strong></button><button onClick={()=>go('accounts')}>Efectivo <strong>{money(balances.cash)}</strong></button><button onClick={()=>go('investments')}>Inversiones <strong>{money(marketValue(investments))}</strong></button></div><div className="stat-grid"><Stat label="Balance del mes" value={balance} icon={<Wallet size={20}/>} tone="dark" foot={balance>=0?'Vas por buen camino':'Tus gastos superan tus ingresos'}/><Stat label="Ingresos" value={income} icon={<ArrowDownLeft size={20}/>} tone="green" foot={`${monthly.filter(x=>x.kind==='income').length} movimientos este mes`}/><Stat label="Gastos" value={expenses} icon={<ArrowUpRight size={20}/>} tone="peach" foot={`${monthly.filter(x=>x.kind==='expense').length} movimientos este mes`}/></div>
        ...
        {tab==='investments'&&<InvestmentsPanel investments={investments} onSave={saveInvestmentLocal} onDelete={deleteInvestmentLocal}/>}
    ...
    {modal&&<div className="modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget){setModal(null);setNotice('')}}}>
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title">
        ...
        {modal==='transaction'?<form onSubmit={saveTransaction}>
          ...
          <div className="form-row"><label>{form.kind==='transfer'?'Desde':'Cuenta'}<select value={form.account} onChange={e=>setForm({...form,account:e.target.value as Transaction['account']})}><option value="bank">Banco</option><option value="cash">Efectivo</option></select></label>{form.kind==='transfer'?<label>Hacia<select value={form.to_account??'cash'} onChange={e=>setForm({...form,to_account:e.target.value as Transaction['to_account']})}><option value="cash">Efectivo</option><option value="bank">Banco</option></select></label>:<label>Fecha<input type="date" required value={form.occurred_on} onChange={e=>setForm({...form,occurred_on:e.target.value})}/></label>}</div>
