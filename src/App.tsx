  ...
  return <div className="app">
    ...
    <aside className={`sidebar ${mobileOpen?'sidebar-open':''}`}>
      ...
      <nav className="nav">
        ...
        <button className={tab==='analysis'?'active':''} onClick={()=>go('analysis')}><Activity size={19}/> Análisis</button>
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {tab==='dashboard'&&<>
          ...
          <nav className="dashboard-paths" aria-label="Explorar tus finanzas">
            <button type="button" className="dashboard-path path-analysis" onClick={()=>go('analysis')}><span className="path-icon"><Activity size={22}/></span><span className="path-copy"><small>ENTENDER</small><strong>Explorar mis gastos</strong><span>{groups.length?'Mayor categoría: '+groups[0].category+' · '+money(groups[0].total):'Gráficos y categorías del mes'}</span></span><ArrowRight size={19}/></button>
            <button type="button" className="dashboard-path" onClick={()=>go('accounts')}><span className="path-icon"><Landmark size={22}/></span><span className="path-copy"><small>UBICAR</small><strong>Mis cuentas</strong><span>Bancos y efectivo por separado</span></span><ArrowRight size={19}/></button>
            <button type="button" className="dashboard-path" onClick={()=>go('budgets')}><span className="path-icon"><Target size={22}/></span><span className="path-copy"><small>PLANIFICAR</small><strong>Presupuestos</strong><span>{totalBudget?money(totalBudget)+' planificados este mes':'Crea tu primer límite mensual'}</span></span><ArrowRight size={19}/></button>
            <button type="button" className="dashboard-path" onClick={()=>go('goals')}><span className="path-icon"><Wallet size={22}/></span><span className="path-copy"><small>AVANZAR</small><strong>Objetivos</strong><span>{goals.length?goals.length+' objetivos en marcha':'Marca una meta de ahorro'}</span></span><ArrowRight size={19}/></button>
          </nav>
          <section className="card recent"><div className="card-heading"><div><span className="section-kicker">ACTIVIDAD RECIENTE</span><h2>Últimos movimientos</h2></div><button className="text-link" onClick={()=>go('transactions')}>Ver todos <ArrowRight size={16}/></button></div><TransactionList accountLabels={accounts} items={monthly.slice().sort((a,b)=>b.occurred_on.localeCompare(a.occurred_on)).slice(0,3)} onEdit={openTransaction} onDelete={deleteTransaction} onAdd={()=>openTransaction()}/></section>
