...
export type Field = 'date'|'amount'|'debit'|'credit'|'description'|'description2'|'merchant'|'category'|'type'|'reference'|'currency';
...
const aliases:Record<Field,string[]>={
 date:['fecha operacion','fecha movimiento','fecha transaccion','fecha','date completed utc','completed date','booking date','date','datum','buchungstag','date operation','data operacao','data','fecha valor','value date'],
...
export function detectColumns(headers:Cell[]):ColumnMap{
 ...
 for(const field of ['date','debit','credit','amount','description','merchant','description2','category','type','reference','currency'] as Field[]){
  let best=0,index=-1;
...
export async function parseSheets(sheets:Sheet[],fileName:string,existing:Transaction[],account:'bank'|'cash',options:ImportOptions={}):Promise<ImportPreview>{
 ...
 for(const sheet of sheets){
  ...
  for(let i=header+1;i<sheet.data.length;i++){
   ...
   const date=dateString(row[columns.date]??null),debit=columns.debit===undefined?null:amountNumber(row[columns.debit]??null),credit=columns.credit===undefined?null:amountNumber(row[columns.credit]??null),amount=columns.amount===undefined?null:amountNumber(row[columns.amount]??null);
   const hasDebit=debit!==null&&Math.abs(debit)>.004,hasCredit=credit!==null&&Math.abs(credit)>.004,type=normal(read(row,'type'));
   const expenseType=/\b(?:expense|debit|card payment|cargo|gasto|salida|pago|lastschrift|belastung|debito|sortie)\b/.test(type),incomeType=/\b(?:income|credit|abono|ingreso|entrada|deposit|gutschrift|credito|entree)\b/.test(type);
