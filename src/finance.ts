...
export function accountBalances(transactions:Transaction[],settings:AccountSetting[],investments:Investment[]){
  ...
  for(const t of transactions){const account=t.account??'bank',amount=Number(t.amount);if(t.kind==='income')balances[account]+=amount;else if(t.kind==='expense')balances[account]-=amount;else{balances[account]-=amount;if(t.to_account)balances[t.to_account]+=amount}}
  for(const holding of investments)if(holding.funding_account!=='outside')balances[holding.funding_account]-=Number(holding.units)*Number(holding.average_cost);
  return balances;
}
