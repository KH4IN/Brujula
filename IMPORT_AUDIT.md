...
Se ejecutaron `npm test` (14 pruebas, formatos bancarios **sintéticos**), `npm run build` y `npm audit --omit=dev --audit-level=high` (0 vulnerabilidades reportadas). Las pruebas incluyen dos operaciones idénticas, reimportación, referencias, distintos separadores, codificaciones, fechas inválidas, tipos contradictorios, categorías y otras divisas. No se usaron extractos oficiales de todos los bancos ni una prueba con dos cuentas reales; la compatibilidad absoluta con los miles de formatos cambiantes no puede verificarse con esta evidencia.
