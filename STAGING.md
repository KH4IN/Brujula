...
La rama `staging` contiene su propia `.env.production` con URL y clave **publicable** del proyecto de pruebas, su `vercel.json` con CSP dirigida a ese proyecto y una protección en `src/data.ts` que rechaza la URL productiva. Esos tres archivos son específicos del entorno y **no deben fusionarse en `main`**. Al publicar funcionalidades, llevar solo los commits o archivos de la función validados en pruebas a una PR contra `main`; revisar el diff para excluir configuración de `staging`.
...
- Las cinco tablas financieras estaban vacías, con RLS activo; el asesor de seguridad del proyecto de pruebas no devolvió avisos en ese momento.
- Vercel generó un despliegue `READY` para el commit `0ce24c78`. La portada del alias respondió 200 y su cabecera CSP permitió la conexión al proyecto de pruebas.
...
Antes de cada publicación en producción: pruebas, build, revisión de los movimientos y sesiones afectados, diff entre ramas y referencia de rollback. Nunca copiar la `.env.production` o CSP de `staging` a `main`.
