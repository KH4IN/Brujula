...
Web instalable (PWA) adaptable a móvil y ordenador. Registra movimientos de banco y efectivo, traspasos, presupuestos, objetivos de ahorro e inversiones manuales. Muestra el reparto por categorías, el gasto diario, el patrimonio estimado y un detalle interactivo. Importa Excel `.xlsx`, CSV y TSV con revisión previa, y exporta movimientos a CSV. Cada cambio se guarda primero en el dispositivo y puede sincronizarse con Supabase mediante una cuenta. El registro permite correo y contraseña repetida; el acceso ofrece contraseña o enlace de correo. Si Supabase exige confirmación por email, se necesita recibir ese correo para activar la cuenta. La opción «Continuar con Google» usa Google como proveedor de **Supabase Auth** en producción; el fundador informa que pudo acceder con ella. El código actual no utiliza el SDK de Firebase para iniciar sesión. Consulta `supabase/AUTH_SETUP.md`. La web oficial es https://brujula-finanzas-gamma.vercel.app/; la rama `main` se publica allí. Las pruebas de cambios se realizan antes en la [vista previa aislada](STAGING.md).
...
Los datos se actualizan inmediatamente después de cada cambio local. Si hay una cuenta iniciada, los cambios pendientes se envían al recuperar conexión o volver a abrir la app; también hay un botón de sincronización manual. Al conectar una cuenta por primera vez se transfieren los datos de invitado de **ese navegador y dominio** a ese usuario. Se recuperan además movimientos propios detectables de una versión local anterior, sin incorporar movimientos de ejemplo. El indicador muestra cuándo los cambios siguen pendientes y cuándo terminaron de sincronizarse. Los datos de otras cuentas quedan separados en el mismo dispositivo; los almacenados en otro navegador o en la PWA instalada tienen que sincronizarse desde allí, iniciando sesión allí también. Si se edita el mismo movimiento en dos dispositivos a la vez, prevalece el último cambio enviado. No hay conexión bancaria ni cotizaciones automáticas.
...
La dirección de uso es la producción; las pruebas de cambios se hacen en el [entorno aislado](STAGING.md), cuyo acceso puede pedir cuenta de Vercel. La autenticación de Brújula/Supabase protege los datos de cada usuario. Publicar código en `main` no modifica las reglas de protección de Vercel. Antes de publicar, `npm test` y `npm run build` deben terminar bien; el workflow `.github/workflows/ci.yml` los ejecuta en las pull requests y en cada push a `main`. Conserva cada cambio en un commit y revisa la pull request antes de fusionarla. Vercel publica automáticamente la rama `main`.
