...
- En Supabase → Authentication → URL Configuration, establecer como Site URL `https://brujula-finanzas-gamma.vercel.app/` y añadir la misma dirección a Redirect URLs. Esa URL está comprobada como pública; `brujula-finanzas-kh4ins-projects.vercel.app` solicita iniciar sesión en Vercel y no debe ser la dirección principal de acceso mientras esté protegida. Para desarrollo, añadir solo los orígenes locales que vayas a usar.
...
1. En Google Cloud del proyecto `brujula-kh`, crear un cliente OAuth de tipo **Aplicación web**. Configurar el origen JavaScript `https://brujula-finanzas-gamma.vercel.app` y la URI de redirección autorizada `https://nvbkftnithuduyazhidc.supabase.co/auth/v1/callback`. Completar la pantalla de consentimiento si Google la requiere; en modo de prueba, añadir las cuentas de prueba.
...
4. El propietario ha indicado que configuró Google y los dominios; `VITE_GOOGLE_OAUTH_ENABLED=true` muestra el botón de acceso en producción. Comprobar el flujo real desde la URL pública gamma con una cuenta de prueba y, antes de usar una cuenta con movimientos, confirmar que los datos están en el UUID correcto. Mostrar el botón no confirma que la configuración externa funciona.
