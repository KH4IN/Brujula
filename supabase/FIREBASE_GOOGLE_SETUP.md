...
Brújula permite entrar con correo y contraseña mediante **Supabase Auth**, en la misma cuenta UUID que utiliza el código por correo. Para crear o completar una cuenta con contraseña, primero solicita un código con `signInWithOtp`, lo verifica y luego establece la contraseña con `updateUser`. Esto sirve también cuando el correo ya tenía una cuenta: no se solicita un segundo registro ni se anuncia falsamente un código a partir de una respuesta ofuscada de `signUp`. Haber activado el proveedor Email/Password en Firebase **no lo conecta automáticamente** con los datos de Supabase. No crear simultáneamente una segunda cuenta Firebase con el mismo correo: su UID sería diferente y no tendría acceso a las filas protegidas por RLS.

En Supabase → Authentication → Providers → Email, mantener activados el registro por email y la confirmación del correo. Las plantillas **Confirm sign up** y **Magic link** deben contener `{{ .Token }}` para que se pueda introducir el código en la app. El enlace de **Reset password** debe conservar `{{ .ConfirmationURL }}` para que el usuario pueda cambiar su contraseña al volver a la app; comprobar el redirect público en URL Configuration. El servicio de correo predeterminado de Supabase no envía a direcciones ajenas al equipo y no es suficiente para un registro público; configurar SMTP antes de abrir el acceso al público.
